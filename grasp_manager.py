...
GRIPPER_OFFSET_IN_CAM = np.array([0.0, 0.05, 0.10])
...
class GraspManager:
    ...
    def grasp_simple(self, target_name='apple'):
        ...
        print("[Grasp Simple] 移动至标准观测位姿以保障标定矩阵有效 (消除俯仰角投影畸变)...")
        self.arm.set_pose(0.20, 0.0, 0.25, pitch=1.5708, yaw=0.0, wait=True, duration=2.5)
        time.sleep(1.0)
        ...
        if not result['success']:
            ...
            return {'success': False, 'action_completed': False}#raise RuntimeError(f"感知失败: {error_msg}")
        ...
        print(f"[Grasp Simple] 目标绝对物理坐标已锁定 (基座系): X={target_x:.4f}, Y={target_y:.4f}, Z={target_z:.4f}")

        # 物理工作空间提示 (已应要求解除强制拦截)
        target_radius = np.hypot(target_x, target_y)
        if target_radius > 0.45 or target_z < -0.05 or target_z > 0.40:
            print(f"[Warning] 目标点可能超限！距基座平距 {target_radius:.3f}m, 高度 {target_z:.3f}m。系统将无视限制，尝试强行下发指令。")

        print("[Action] 移动至目标上方预抓取点 (Z + 0.10m)...")
        success = self.arm.set_pose(target_x, target_y, target_z + 0.10, wait=True, duration=2.5)
        if not success: raise RuntimeError("机械臂上方位姿移动指令执行异常")

        print("[Action] 下放至夹取高度 (补偿 -0.03m)...")
        success = self.arm.set_pose(target_x, target_y, target_z - 0.03, wait=True, duration=1.5)
        if not success: raise RuntimeError("机械臂下放指令执行异常")

        print("[Action] 闭合夹爪 (应用极轻柔抓取力度 0.5)...")
        ...
        print("[Action] 抓取完成，回归零点...")
        self.arm.go_home(speed_pct=15, duration=3.0)
